...
module.exports = class Artist {
  ...
  static async updateById(id, { artist, albumName, isFavorited }) {
    ...
    console.log('HHHHUUUUGE STRIIIIING', existingArtist);
    if (!existingArtist) {
    ...
    const newalbumName = albumName ?? existingArtist.album_name;
    const newisFavorited = isFavorited ?? existingArtist.is_favorited;
    const { rows } = await pool.query(
